...
class ChemistryRecord : public StudentRecord
{
protected:
	string firstName;										// First name
	string lastName;										// Last name
	vector<ChemistryUnitRecord*> units;						// Vector to hold courses taken
	
public:

	~ChemistryRecord(){}

	ChemistryRecord(string first, string second)
	{
		firstName = first;
		lastName = second;
	}
	...
	{
		ChemistryUnitRecord *newrecord = new ChemistryUnitRecord();
	...
	{
		for (vector<ChemistryUnitRecord*>::iterator it = units.begin() ; it != units.end(); ++it)
		{
